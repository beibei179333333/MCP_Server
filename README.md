...
1. **下载并解压**：点这个链接下载压缩包，下载后右键 →「全部解压缩」：
   https://github.com/beibei179333333/MCP_Server/archive/refs/heads/claude/group-member-export-tool-sWRs7.zip
2. 进解压出来的文件夹，**双击 `setup.bat`**。
   它会**自动安装 Python（若没有）→ 自动装依赖 → 启动 → 自动打开浏览器** `http://localhost:8000`。
   全程不用你操作，等它跑完弹出网页即可。

然后在网页 **② 过滤选项 → 高级设置** 里粘贴 **密钥(Token)** → 粘贴群链接 → 开始导出 → 下载 CSV / Excel / JSON。

> - 已经装过 Python 的，也可以直接双击 `run.bat`（更快，跳过装 Python 这步）。
> - 右上角可切换 **中文 / 越南语**。
> - 停止：关闭那个黑色命令行窗口；以后用再双击 `setup.bat`（或 `run.bat`）。
> - 若 Windows 弹 “已保护你的电脑”，点 **更多信息 → 仍要运行**（脚本没签名，正常现象）。
> - 万一自动装 Python 没成功（极少数），按窗口提示去 https://www.python.org/downloads/ 手动装（勾选 “Add Python to PATH”），再双击一次。
